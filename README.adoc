...
is doing at particular point of time. 

When an FSM changes *_State_*, it goes from doing one thing to doing another thing. 

Over time, when certain events occur, the Machine changes from one State to another. 
These events are called *_Triggers_* and a change in State is called a *_Transition_*. Sometimes there are certain *_Actions_* (behaviour/computation) associated with 
...
* it can undertake "an activity" which naturally comes to an end (this ending might causes a Trigger)
