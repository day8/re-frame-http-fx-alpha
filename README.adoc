...
* optionally, if the application is to allow th user to cancel the request (via a button?) then capture the `:id` of teh request and assoc it into `app-state` for easy access.
...
* trigger `fetch` to cause the transition to `waiting`, with the `fetch` being an action associated with the transition.
...
[source, Clojure]
----
(fn [cofx [_ request-state]]
  (let [db (:db cofx)
        {:keys [request-id context]} request-state]
    {:http  {:trigger :fetch
             :request-id request-id}
     :db    (-> db
              (assoc-in (conj (:path context) :request-id) request-id)
...
(fn [cofx [_ request-state]]
  (let [db (:db cofx)
        {:keys [request-id response context]} request-state
        path (:path context)
...
----
(fn [cofx [_ request-state]]
  (let [{:keys [request-id]} request-state]
    {:http {:trigger :done
            :request-id request-id}}))
----
...
(fn [cofx [_ request-state]]
  (let [db (:db cofx)
        {:keys [request-id context problem response]} request-state
        path (:path context)
