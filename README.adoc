...
Initialize `app-state` for this request.
...
* trigger `send` to cause the tranistion to `in-waiting`, with the `fetch` being the tranistions associated action. 
