...
* The Logical State.. A single value which captures the logical state of the FSM
  which , in turn, controls what "the machine" is currently doing (ie. what *_Logical State Handler_* is executing). 
* `request-state` is all the detail which this library maintains about a request when it is being performed, including 
...
* `presentation-state` - a "projection" of `request-state` which is stored across in `app-db`, at the nominated path. It is a mirrors 
   of those aspects of the `request-state` which are need to present a UI. So, for example, there might need to be a `:loading?` value      set to true to indicate that the busy twirly should be kept up.  Or perhaps a `:retrying?` flag might need to be "projected" from the    `reguest-state` so that, again, the UI can show the user what is happening.  

The `request-state` disappears when the request finishes. But the `presentation-state` persists. Ultimately, the most important part of this `presentation-state` is the response data itself. But there will be other information alongside it. For this reason, `presentation-state` is normally a map of values with a key for `response`, but it has other values.
...
XXX the receipies for each of the Logical State Handlers ... 
