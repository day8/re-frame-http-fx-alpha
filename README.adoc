...
> This library is under development. We're interested in any ideas or experienced-based critiques.
...
* `request-state` is operational detail about a request, maintained by this library 
   to get its job done. So, it is the memory of the FSM and it exists only for the lifetime 
   of the request. It includes such data as an id, the currently logical
   state of the FSM, the original request, number of retries, the trace history through
   the FSM, etc. 

   It is provied to each *_Logical State Handlers_* and they can mutate it. 
...
* `presentation-state` - this application state (not library state) and it is stored in `app-db`. It is typically a map. 
  It is placed a a particular path. 
  It is maintained by the *_Logical State Handlers_* 
  that you write. Typically, the `in-setup` handler initialises this state and it is 
  then maintained across the request 
  process as it runs through the "machine" (FSM). It will be something 
  of a "a materialised view" of the full `request-state` and will be sufficient to render a UI for the user. 
  This state outlasts the request - particularly the most important part, the processed response to 
  the request.  

XXX show example of `presentation-state` map. 

XXX consider what else needs to happen to help `re-frame-async-flow`

So, I'd like to stress two points already made:
   - lifetime: `presentation-state` exists for as long as your applciation code says it should - it persists. Whereas 
     `request-state` is created and destroyed by this library - it is a means to an ends - it is transatory. 
   - during the request process, `request-state` tends to be authorative. : `presentation-state` is something 
...
I'd like to stress a point already made. `presentation-state` 
  . a "a limited projection" of the `
  will be a "limited projection" 
  of `request-state` which is stored across in `app-db`, at the nominated path. It is a mirrors 
