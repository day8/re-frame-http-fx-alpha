...
> But this README/tutorial is getting close to being finished. It says how the library will work. + 
> If you have time/interest, have a read and give us your ideas or critique. Or, more causually, tweet me @wazound
...
* the names on those lines are the *_Triggers_* (the event which causes the Transition to happen)
...
* `request-state` is detail about a request, maintained by this library 
   ...
   XXX issac where is this state? Currently stored in an atom help within the library. But 
...
* write your Logical State Handlers (or simply use the default handlers provided)
...
Initialize `app-state` for this request. Views subscribed to this state will 
then render the UI, allowing the user to see feedback. 

XXX coincidently, a panel might change .... perhaps the user clicked a button to "view foobars", so the application will change panels to the foobars one, AND also kickoff the request to get the foobars. 

.Do this:
* establish initial `app-state` at the nominated path 
* obtain the id of the request, and store it in `app-state` so it can be used later when the user clicks the "cancel" button. 
