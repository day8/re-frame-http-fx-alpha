...
* design `app-state` and the views which render it (or simply use the default design we suggest)
* write your Logical State Handlers (or simply use the default handlers we provide)
...
Prepare your `app-state` for the request. Views which are subscribed to this state will then render the UI, allowing the user to see feedback. 
...
In this state, we are waiting for an HTTP reponse (after the `fetch` is launched). 
...
.We are in this state because of one of the following reasons:
* no outright failure, but too many retries (see `:http/history` XXX for what happened)  
* unrecoverable server error
* an error occured when processing a response
