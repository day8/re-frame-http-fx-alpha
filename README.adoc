...
JSON and transit into EDN, before ending up at the right place in `app-db`. Etc.

So, with two lines of code - simple ones at that - you'll get a robust HTTP
...
*_Logical State_* in the FSM.  To help you, this library offers "a recipe"
...
Finally, let's talk about state. Each FSM instance has some
...
If you look back, you'll see the profile was registered with `:default? true` and 
that means it will be used by default, if no profile is explicitly provided. 

Here's an explicit example of using that profile we registered above:
[source, Clojure]
----
{:http {:get      "http://api.endpoint.com/articles/"
        :path     [:put :response :data :at :this :path :in :app-db]
