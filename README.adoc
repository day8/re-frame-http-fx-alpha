...
> This library is under development. Best not to read this yet, unless you want to provide ideas/critique. 
...
When each profile holds a `:http/values` map and we must combine multiple of them, how should 
we "combine" the many into one map to be our `:http` specification?  
...
----
`merge` accumulates the key/value pairs,   
with any values in "later" maps overwriting the values in "earlier" ones. 
...
Remember that `merge` is shallow. Consider:
[source, Clojure]
----
(def map1 {a: {:aa 1})
(def map2 {a: {:cc 1})
...
----
You might be tempted to thing the result should be: 
`{a: {:aa 1 :cc 1}}`. 

But it is actually: `{a: {:cc 1}}`. 

The `:a` value of `map2` which is `{:cc 1}` simply replaces **completely** the earlier value of `{:aa 1}`. The maps `{:aa 1}` and `{:cc 1} are not themselves merged because `merge` is shallow and does not act deeply/recusively on the values themselves.

== Beyond Default `merge` Behaviour

This default `merge` behaviour is often good enough, particularly when the profiles being combined don't overlapping
keys. 

But, sometimes, when profiles have overlapping keys it is useful to take control about how the `values` 
are combined. 

Sometimes this default `merge` behaviour is not enough and this library lets you take change of the 
combining process for each potential key in `.
...
There's three kinds of state: 

* The Logical State (of the FSM). This is just a single value which says what state we 
  are in within the FSM which, in turn, controls what "the machine" is currently 
  doing (ie. what `Logical State Handler` is executing). 
* `request-state` is all the detail which this library maintains about a request, including 
   an id, the original request, number of retries, the trace history through the FSM, etc.  This state exists only for the time the 
* `presentation-state` - a "projection" of `request-state` which is stored in `app-db`, at some path. Two parts: it tracks those aspects of the request which need to be reflected in the UI.  So, for example, there might need to be a `:loading?` value set to true to indicate that the busy twirly is kept up.  Or perhaps a `:retrying?` flag might need to be "projected" from the `reguest-state` so that, again, the UI can show the user what is happening.  

The `request-state` disappears after the request happens. But the `XXX-state` persists. Ultimately, the most important part of this `presentation-state` is the response data itself. But there will be other information alongside it. 

The `presentation-state` is put at the path nominated within `app-db`.

The `presentation-state` is managed by your `Logical State Handlers`. You control what data is projected from the `request-state` across into the `presentation-state`. Because you, the application programmer, knows what you want to set within `app-db`. You know how you want the UI to render the state of the request process.
...
XXX we recomend that the path does not contain just request's response. Indeat it contains 

Logical State  is the easy one to talk about. 

  
But this data is not stored in `app-db`. 
