...
> This library is still under development, so don't try to use it yet. + 
> But you can still give us feedback. Influence us with your ideas and usecases!  + 
> We use README-driven development, so this document is getting quite complete. + 
> Have a read and open an issue with your thoughts, ideas or critique. Or, more casually, tweet me @wazound
...
This is our experienced-based improvement on the original [re-frame-http-fx library](https://github.com/Day8/re-frame-http-fx).
...
Here's a re-frame event handler returning an effect keyed `:http` (see the last two lines):
[source, Clojure]
----
(ref-event-fx 
...
That `:http` effect will do a GET and place any response data into
...
*_right defaults_* for your application. Let us begin the learning now ...
