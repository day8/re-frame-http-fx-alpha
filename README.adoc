...
* `request-state` is detail about a request, maintained by this library 
   ...
   It is provided to each *_Logical State Handler_*. 
...
* `app-state` - this is application state (not library state). 
  It is typically a map which exists at a particular path within `app-db`. 
  It is maintained by the  *_Logical State Handlers_* that you write and the path is often supplied 
  as a part of the request itself. Typically,
...
----
{
  ...
  :result    nil
  :retries   0
  :description  "Loading expired thingos"
}
----

This is the data in `app-db` which the view functions might subscribe to, in order to render the 
