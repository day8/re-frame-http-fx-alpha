...
That new key `:profiles` allows you to nominate a vector of previously registered *_Profile_* `ids`. The 
`:values` from those *_Profiles_* will be added in the request.
...
we want to combine `profile1`, `profile2`, etc. 
...
So far, so good. This process is straightforward while ever the profiles have disjoint keys. But, when there are duplicates,
we need a strategy to "combine" the values for duplicate keys. 

.Here are the rules for combining key values:  
* if both values satisfy `str?`, then `str` is used to combine them
* if both values satisfy `set?` then `clojure.set/union` is used 
* if both values satisfy `map?`, then `merge` is used. (remember merge is shallow).
...
```
because the values for the duplicate `:url` keys are strings, they will be combined with `str`.
...
{:http {:action :GET
        ...
        :profiles [:jwt-token :standard-parms :xyz]}}     ;;  <---- MULTIPLE
----
the final `:http` spec will be a map which is the combination of all the 
maps in nominated `:profiles`, and following the combination rules given above, as if
`special-merge` was used. 
...
Where you need to take detailed control of the "combining" process you can use the libraries API function `merge-profiles`
```
...
* `path-state` - this is the application state which represents the request (not
...
Typically, the `in-setup` LogicalStateHandler initialises the `path-state` map, and it is 
