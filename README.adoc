...
*_Transitions_* cause a FSM to move from one Logical State to another - from one behaviour to another. Over time - and I really would like to stress this time bit - events occur whcih trigger Transitions. That means a FSM goes from doing one thing, to doing another thing.
...
This specification offers a lot of flexibility. But we don't want to repest this 
much everytime. Particularly because, time after time,
we'll want the same headers, or params and `Logical State handers`. 

== Profiles

A *_profile_* captures a fragment of an an `:http` specification for later reuse. 
It is a way of factoring out and capturing commonality.

You can "register" one or more *_profiles_* and you typically do this on application 
startup. 

It is important to realise that an `:http` specification is just data (a map). So, to 
"capture a fragment" of a specification means simply to capture part of that specification 
data. And if you think that sounds pretty simple, you'd be right.
...
Once you have registered a profile, you can use it within an `:http` effect like this:
```clj
