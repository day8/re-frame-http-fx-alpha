...
> However ... we practice RDD (README-Driven Development) and this document describes + 
> what you'll be getting. You can *_influence our design_* by reading it + 
...
This library formalises that process as *_a Machine_* (the abstract concept of something which
*_does something_*). And, specifically, it uses a Finte State Machine (FSM), which is a 
Machine with a fixed, finite number of *_States_*. 

When a *_Machine_* is in one of these *_State_*, it has discrete responsibilities, 
concerns and behaviours. Because a FSM can only be in one State at a time, States are how we reason about what a Machine 
is doing at particular point of time. When an FSM changes *_State_*, it goes from
doing one thing to doing another thing. 

Over time, when certain events occur the Machine moves from one State to another. These events are called *_Triggers_* and the 
State changes are called *_Transitions_*. Sometimes there are certain *_Actions_* (behaviour/computation) associated with 
a *_Transition_*. But, to repeat, the significant thing about a State change is that the Machine goes from
doing one thing to doing another thing.

.Being in a State:
* can involve passive waiting (for some eternal event to Trigger a Transition)
* can mean undertaking "an actvity" which will come to an end, and this "end" is a Triggering event.
* can mean undertaking "an activity" which does not come to an end, and the Machine only moves on because of an external event 
* can be a https://www.uml-diagrams.org/state-machine-diagrams.html#pseudostate[a PseudoStates] which involves a choice about the next Trigger/Transition/State. 

This library's FSM has all of this. `Problem` for example is very much a choice `PseudoState`. `Waiting` is a passive State. 
